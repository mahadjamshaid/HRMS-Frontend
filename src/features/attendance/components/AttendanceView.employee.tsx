    ...
    const employeeColumns: Column<AttendanceRecord>[] = [
        {
            ...
            render: (record) => (
                <>
                    <p className="text-sm font-black text-slate-900">{formatLongDate(record.date)}</p>
                    <p className="text-[10px] font-bold text-slate-400 uppercase tracking-widest mt-1">{formatWeekday(record.date)}</p>
                </>
            )
        },
        {
        ...
        },
        {
            ...
            render: (record) => <WorkHoursWidget checkInTime={record.checkInTime} checkOutTime={record.checkOutTime} />
        }
    ];
